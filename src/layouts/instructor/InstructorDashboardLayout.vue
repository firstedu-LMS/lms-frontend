<template>
    <div class="flex justify-end">
      <aside v-if="sideBar" class="h-screen fixed top-0 left-0 border-r border-[#e2e8f0] z-50 bg-white pr-[10px] py-2">
        ...
        <ul>
          <li :title="item.name" class="my-6 font-semibold" v-for="item in sideItems" :key="item.name">
            <router-link :to="{name : item.path}" class="flex items-center" @click="sideBar = false">
              <span class="material-icons" style="margin-right: 8px; background-color: rgb(249 250 251 / var(--tw-bg-opacity));z-index: 1; padding-left: 12px;">{{ item.icon }}</span>
                <span style="z-index: 0.5;">{{ item.name }}</span>
            </router-link>
          </li>
          <li class="font-semibold border-t border-t-gray-3">
            <button @click="logout" class="flex items-center my-5 text-red">
                ...
                <span style="z-index: 0.5;">Logout</span>
            </button>
          </li>
        </ul>
      </aside>
      <div class="transition-all ease-in-out">
        <nav class="sticky top-0 z-40 flex justify-between w-full p-2 text-white shadow-lg bg-blue-2">
