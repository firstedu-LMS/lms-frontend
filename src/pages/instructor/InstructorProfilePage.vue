<template>
    <div>
        <div v-if="user.image && user.image.image && previewImage" @click.self="previewImage = false" class="fixed top-0 left-0 z-50 w-screen h-screen bg-black bg-opacity-80">
            <div class="absolute sm:w-[30%] w-[90%] top-1/2 left-1/2" style="transform: translate(-50% , -50%);" >
                <img :src="filePath.imagePath(user.image.image)" alt="">
                <input @change="editImage" type="file" class="hidden" ref="profileImage">
        ...
        <div class="z-10 flex flex-wrap items-center justify-center py-12">
            <div v-if="user" @click.self="edit = editName = false;" class="flex justify-center w-full sm:w-[45%]">
                ...
                <p v-else class="w-[180px] bg-gray-1 h-[180px] rounded-full border-[0.4rem] border-blue-3 flex items-center justify-center text-[120px]">{{ user.name.charAt(0) }}</p>
            </div>
        ...
        <div @click.self="edit = editName = false;" class="w-full sm:flex justify-evenly bg-[#f2efef]">
            ...
            <div class="w-full sm:w-[45%] px-2 pt-4 pb-8">
                ...
                <div class="text-xs sm:text-sm">
                    ...
                    <div :class="classes.inputGroups">
                        <p :class="classes.labels">Finished Courses</p>
                        <p :class="classes.inputs" class="text-center">{{ finishedCourse }}</p>
                    </div>
                    <div :class="classes.inputGroups">
                        <p :class="classes.labels">&nbsp;</p>
                        <p :class="classes.inputs" class="text-center">&nbsp;</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    export default {
        ...
        mounted() {
            ApiService.get('instructors/profile').then((res) => {
                ...
                this.finishedCourse = res.data.data.finishedCourse;
                this.currentCourse = res.data.data.currentCourse;
            }).catch((res) => {
                console.log(res);
            })
        },
        methods : {
            ...
            editInstructor () {
                ...
                ApiService.patch('instructors/profile-update' , obj).then((res) => {
                    console.log(res.data.data);
                    this.instructor.phone = res.data.data.phone;
                    this.instructor.address = res.data.data.address;
                    this.editing = false;
                }).catch((res) => {
                    console.log(res);
                })
            ...
            updateName() {
                ...
                ApiService.patch('instructors/profile-name-update' , obj).then((res) => {
                    console.log(res.data.data);
